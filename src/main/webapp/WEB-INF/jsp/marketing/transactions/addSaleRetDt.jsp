...
<div id="saleRetDtDivId" style="display: none">
		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-xs-10">
					...
					onClick="javascript:addBarcode()"
					href="javascript:void(0)"><span
					class="glyphicon glyphicon-plus"></span>&nbsp;Add Barcode</a>
					
					</div>
					
					<div class="col-sm-4">
					<a class="btn btn-info" style="font-size: 15px" type="button"
					onClick="javascript:saleIssPickup()"
					href="javascript:void(0)"><span
				...
				<!--  	<div class="col-sm-4">
					<a class="btn btn-success" style="font-size: 15px" type="button"
					onClick="javascript:displaySaleRetDtSummary()"
					href="javascript:void(0)"><span
					></span>&nbsp;Summary</a>
					</div> -->
					...
					<div>
						<table id="saleRetDtTblId" data-toggle="table" data-toolbar="#toolbar" data-search="true"
						...
						<input type="hidden" id="saleRetDtPKId" name="saleRetDtPKId" />
					</div>
				...
				<div id="odImgDivId" class="col-xs-2 center-block">
					...
					<div class="panel panel-primary" style="width:100%; height:245px">
						<div class="panel-body">
							...
							<div class="pull-left">
								<table id='stoneDtlsId' style="width:100%" class="line-items editable table table-bordered">  <!--  AskAboutStone details change to stoneRetDtlsID..? -->
								</table>
							</div>
						</div>
					</div>
...
<!-- //hide on page load start here -->
	...
	<div class="row">
		<div class="col-lg-12 col-sm-12">
			<span class="col-lg-12 col-sm-12 label label-default"
				...
				<div>
					<table id="saleRetMetalTableId" data-toggle="table"  data-unique-id="id" >
						<thead>
							<tr class="btn-primary">
		...
		<div class="row">
			<div class="form-group">
				<div class="col-lg-12 col-sm-12">
					...
					<div >
						<table class="table-responsive"  id="saleRetStnDtTabId"  data-toggle="table" 
						...
						<input type="hidden" id="saleStoneDtPKId" name="saleStoneDtPKId" />         <!--  AskAboutStone saleStoneDtPKId change to saleRetStoneDtPKId..? -->
	...
	<div id="toolbarCompDt">
					...
					<div >
						<table  id="saleRetCompDtTabId" data-toggle="table" data-toolbar="#toolbarCompDt"	data-click-to-select="true">
							<thead>
								<tr class="btn-primary">
									<th data-field="state" data-radio="true"></th>
		...
		<div class="form-group">
		 <div class="col-lg-12 col-sm-12">
					...
					<div>
						<table id="saleRetLabDtTabId" data-toolbar="#toolbarLabDt"   data-toggle="table" 
							data-click-to-select="true">
							<thead>
								<tr class="btn-primary">
									<th data-field="state" data-radio="true"></th>
...
var saleRetDtTableRow;
var saleRetDtLockStatus = 'null';
var finalPrice = "";
 $('#saleRetDtTblId').bootstrapTable({}).on(
		'click-row.bs.table',
		function(e, row, $element) {
			
			saleRetDtTableRow = $element.attr('data-index');
			
			$('#saleRetDtPKId').val(jQuery.parseJSON(JSON.stringify(row)).id);
			
			var tempDefaultImage = jQuery.parseJSON(JSON.stringify(row)).image;
			saleRetDtLockStatus = jQuery.parseJSON(JSON.stringify(row)).rLock;
			...
			popSaleRetMetal();
			popSaleRetStoneDetails();
			popSaleRetComponentDetails();
		
			popSaleRetLabDetails(); 
	...
	function popSaleRetDt(flg) {
		
		$("#saleRetDtTblId")
				.bootstrapTable(
						'refresh',
						{
							url : "/jewels/marketing/transactions/saleRetDt/listing.html?mtId="+$('#saleRetMtFormId #id').val()+"&disableFlg="+flg,
						});
	}


	function popSaleRetMetal()
	{
		...
		.bootstrapTable(
				'refresh',
				{
					url : "/jewels/marketing/transactions/saleRetMetalDt/listing.html?dtId="+$('#saleRetDtPKId').val(),
				});	
	}


	function popSaleRetStoneDetails(){
		
		$("#saleRetStnDtTabId")
		.bootstrapTable(
				'refresh',
				{
					url : "/jewels/marketing/transactions/saleRetStnDt/listing.html?dtId="+$('#saleRetDtPKId').val()+"&disableFlg="+disableFlg,
				});	
		
	}

	
	function popSaleRetComponentDetails(){
		
		$("#saleRetCompDtTabId")
		.bootstrapTable(
				'refresh',
				{
					url : "/jewels/marketing/transactions/saleRetCompDt/listing.html?dtId="+$('#saleRetDtPKId').val()+"&disableFlg="+disableFlg,
				});	
		
	}
	
	function popSaleRetLabDetails(){
		$("#saleRetLabDtTabId")
		.bootstrapTable(
				'refresh',
				{
					url : "/jewels/marketing/transactions/saleRetLabDt/listing.html?dtId="+$('#saleRetDtPKId').val()+"&disableFlg="+disableFlg,
				});	
	}
	...
	$('#saleRetStnDtTabId').bootstrapTable({})
	.on(
			'load-success.bs.table',
			function(e, data) {
				var data = JSON.stringify($("#saleRetStnDtTabId").bootstrapTable('getData'));
	...
	function deleteSaleRetDt(e,dtId){
		displayDlg(
				e,
	...
	function deleteDt(dtId){
		...
		$("#modalDialog").modal("hide");
						 ...
						 $.ajax({
								url : "/jewels/marketing/transactions/saleRetDt/delete/"+ dtId + ".html",
								type : 'GET',
...
function displaySaleRetDtSummary(){
		
		$('#mySaleRetDtSummaryModal').modal('show');
		 $('#mySaleRetDtSummaryModal').on('shown.bs.modal', function () {
			 popSaleSummary();
	        });
	...
	function popSaleSummary(){
		...
		$.ajax({
			url:"/jewels/marketing/transactions/saleRetMt/dtSummary.html?mtId="+$('#saleRetMtFormId #id').val(),
			type :'GET',
			dataType:'JSON',
			success: function(data){
	...
	function applyGst(){
		...
		if($('#hsnMast\\.id').val() != '' && $('#saleRetMtFormId #id').val() != ''){
		$.ajax({
			...
			$('#pSgst').val(vSgst);
			 ...
			 $('#pIgst').val(vIgst);
				...
				$.ajax({
					url:"/jewels/marketing/transactions/saleRetMt/addSummaryDetails.html?mtId="+$('#saleRetMtFormId #id').val()+"&fob="+ $('#totalFob').val()+"&sgst="+vSgst
	...
	$('#saleRetDtTblId').bootstrapTable({})
	.on(
			'load-success.bs.table',
			function(e, data) {
				var data = JSON.stringify($("#saleRetDtTblId").bootstrapTable('getData'));
	...
	//	$('#vFinalPrice').val($('#totFinalPrice').val());
		...
		/* $.ajax({
			url:"/jewels/marketing/transactions/saleRetMt/addSummaryDetails.html?mtId="+mtid+"&fob="+ $('#totalFob').val()+"&sgst="+vSgst
			+"&cgst="+ vCgst +"&igst="+vIgst+"&otherCharges="+$('#otherCharges').val()+"&finalPrice="+  $('#totFinalPrice').val(),
			type :'GET',
			success: function(data){
	...
	$('#saleRetDtTblId').bootstrapTable({}).on('load-success.bs.table',
			function(e, data) {
		   
		dtdata = $("#saleRetDtTblId").bootstrapTable('getData').length;
	...
	function addBarcode(){
		saleRetModeFlag = "add";
		 var validator = $( ".saleRetDtForm" ).validate();
			validator.resetForm();
			
		$('#mySaleRetDtModal').modal('show');
		
		setTimeout(function(){
			 $('#saleRetDtMetalTableDivId').css('display','none');
			$('#saleRetDtMetalIdTbl').bootstrapTable('resetView');
		}, 200);
		 ...
		 $('#saleRetDtFormId #purity\\.id').val($('#orderMtDataId #purity\\.id').val());
		 $('#saleRetDtFormId #color\\.id').val($('#orderMtDataId #color\\.id').val());
		
		 $('#saleRetDtDtModalDivId  #design\\.mainStyleNo').focus();
		 $('#design\\.mainStyleNo').removeAttr('readonly','readonly');
		 
		 $('#saleRetDtFormId #purity\\.id').removeAttr('disabled','disabled');
		$('#saleRetDtFormId #color\\.id').removeAttr('disabled','disabled');
