...
<div class="panel panel-primary">
	...
	<label class="col-lg-5 text-left">
				<span style="font-size: 18px;">Order Pickup</span>
			</label>
		...
		<a class="btn btn-xs btn-info" id="orderPickupBackBtnId" style="font-size: 14px" type="button" onclick="javascript:orderPickupBackBtn()"><b>Back</b></a>
		</div>	
	...
	<div class="panel-body">
			...
			<div class="row">
				...
				<div class="table-responsive">
 					<table id="orderPickupTblId" data-toggle="table" data-search="true" data-toolbar="toolbarDt" data-maintain-meta-data="true" data-height="450"  >
					<thead>
			...
			<div id="odImgDivId" class="col-md-2 center-block">
					...
					<div class="panel panel-primary" style="width:100%; height:245px">
						...
						<div class="form-group">
							<div>
								<input type="button" value="Transfer" onclick="javascript:orderPickupTransfer()" id="trfBtnId" class="btn btn-primary">
							</div>
						</div>
...
.ready(
		function(e) {
		 	...
		 	if (window.location.href.indexOf('mtid') != -1) {
						...
						grcmtId = vUrl.substring(window.location.href.indexOf('mtid') + 5,window.location.href.indexOf('&partyId'));

						partyId = vUrl.substring(window.location.href.indexOf('&partyId') + 9,window.location.href.indexOf('&orderTypeId'));

						orderTypeId = vUrl.substring(window.location.href.indexOf('&orderTypeId') + 13);
...
function popBagDt(){
				...
				.bootstrapTable(
						'refresh',
						{
							url : "/jewels/marketing/transactions/orderList/listing.html?orderTypeId="+orderTypeId+"&partyId="+partyId,
						}); 
...
$('#orderPickupTblId').on('check.bs.table', function (e, row) {
...
$('#orderPickupTblId').on('check-all.bs.table', function (e, row) {
...
$('#orderPickupTblId').on('uncheck.bs.table', function (e, row) {
...
$('#orderPickupTblId').on('uncheck-all.bs.table', function (e, row) {
...
$('#orderPickupTblId').bootstrapTable({}).on(
		'load-success.bs.table',
		function(e, data) {
			var data = JSON.stringify($("#orderPickupTblId").bootstrapTable('getData'));
			var vBagPcs = 0.0;
			var vWt = 0.0;
			var i=0;
...
function popSelectedQty(){
	...
	var data = $('#orderPickupTblId').bootstrapTable('getAllSelections');
...
function orderPickupTransfer(){
	...
	$.ajax({
		url : "/jewels/marketing/transactions/orderPickupTransfer.html?bagIds="+$('#pBagIds').val()+"&grcmtId="+grcmtId,
...
function orderPickupBackBtn(){
	

		window.location.href = "/jewels/marketing/transactions/grecMt/edit/"+grcmtId+".html"
