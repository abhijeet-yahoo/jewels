	...
	<div class="panel-body">
			...
			<div class="row">
				...
				<div class="table-responsive">
 					<table id="stockMeltTransferTblId" data-toggle="table" data-search="true" data-toolbar="toolbarDt" data-maintain-meta-data="true" data-height="450"  >
					<thead>
...
.ready(
		function(e) {
		 	...
		 	if (window.location.href.indexOf('mtid') != -1) {
						...
						stkMeltMtId = vUrl.substring(window.location.href.indexOf('mtid') + 5,window.location.href.indexOf('&locationId'));
...
function popBagDt(){
				...
				$("#stockMeltTransferTblId")
				.bootstrapTable(
						'refresh',
						{
							url : "/jewels/marketing/transactions/stockMeltingTransfer/listing.html?deptId="+deptId,
						}); 
...
$('#stockMeltTransferTblId').on('check.bs.table', function (e, row) {
...
$('#stockMeltTransferTblId').on('check-all.bs.table', function (e, row) {
...
$('#stockMeltTransferTblId').on('uncheck.bs.table', function (e, row) {
...
$('#stockMeltTransferTblId').bootstrapTable({}).on(
		'load-success.bs.table',
		function(e, data) {
			var data = JSON.stringify($("#stockMeltTransferTblId").bootstrapTable('getData'));
			var vBagPcs = 0.0;
			var vWt = 0.0;
			var i=0;
...
function popSelectedQty(){
	...
	var data = $('#stockMeltTransferTblId').bootstrapTable('getAllSelections');
...
function popStockTrfToMelting(){
	...
	$.ajax({
		url : "/jewels/marketing/transactions/stockTransferToMeltingFromPickup.html?vBarcodeIds="+$('#vMtId').val()+"&stockMeltMtId="+stkMeltMtId,
...
function stockMeltingBackBtn(){
		...
		window.location.href = "/jewels/marketing/transactions/stockMeltingMt/edit/"+stkMeltMtId+".html"
