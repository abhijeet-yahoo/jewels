...
<div class="panel panel-primary">
	...
	<label class="col-lg-5 text-left">
				<span style="font-size: 18px;">Stock Melting Details</span>
			</label>
		...
		<a class="btn btn-xs btn-info" id="stockMeltingBackBtnId" style="font-size: 14px" type="button" onclick="javascript:stockMeltingBackBtn()"><b>Back</b></a>
		</div>	
	...
	<div class="panel-body">
			...
			<div class="row">
				...
				<div class="table-responsive">
 					<table id="stockMeltPickupTblId" data-toggle="table" data-search="true" data-toolbar="toolbarDt" data-maintain-meta-data="true" data-height="450"  >
					<thead>
							<tr class="btn-primary">
								...
								<th data-field="purityVal" data-align="left" data-sortable="true" data-filter-control="input">Kt-Col</th>
			...
			<div id="odImgDivId" class="col-md-2 center-block">
					...
					<div class="panel panel-primary" style="width:100%; height:245px">
						...
						<div class="form-group">
							<div>
								<input type="button" value="Transfer" onclick="javascript:popStockIssueToMelting()" id="trfBtnId" class="btn btn-primary">
							</div>
						</div>
		...
		<input type="hidden" name="vDtId" id="vDtId" />
	</div>		
...
.ready(
		function(e) {
		 	...
		 	if (window.location.href.indexOf('mtid') != -1) {
						...
						meltingMtId = vUrl.substring(window.location.href.indexOf('mtid') + 5);
						}
...
function popBagDt(){
				...
				$("#stockMeltPickupTblId")
				.bootstrapTable(
						'refresh',
						{
							url : "/jewels/marketing/transactions/stockMeltingApproval/listing.html?approvalFlg=false",
						}); 
...
$('#stockMeltPickupTblId').on('check.bs.table', function (e, row) {
...
$('#stockMeltPickupTblId').on('check-all.bs.table', function (e, row) {
...
$('#stockMeltPickupTblId').on('uncheck.bs.table', function (e, row) {
...
$('#stockMeltPickupTblId').on('uncheck-all.bs.table', function (e, row) {
...
$('#stockMeltPickupTblId').bootstrapTable({}).on(
		'load-success.bs.table',
		function(e, data) {
			var data = JSON.stringify($("#stockMeltPickupTblId").bootstrapTable('getData'));
			var vBagPcs = 0.0;
			var vWt = 0.0;
			var i=0;
...
function popSelectedQty(){
	...
	var data = $('#stockMeltPickupTblId').bootstrapTable('getAllSelections');
...
function popStockIssueToMelting(){
	...
	$.ajax({
		url : "/jewels/manufacturing/transactions/stockIssueToMelting/save.html?vDtId="+$('#vDtId').val()+"&meltingMtId="+meltingMtId,
...
function stockMeltingBackBtn(){
	
	 window.location.href = "/jewels/manufacturing/transactions/meltingMt/edit/" + meltingMtId	+ ".html"
