...
<div class="panel panel-primary" style="width: 100%">
	<div class="panel-heading">
		<span style="font-size: 18px;">PD Metal Addition</span>
	</div>
	...
	<div class="panel-body">
		<div class="col-xs-10">
			...
			<form:form commandName="pdcTranMt"
				action="/jewels/manufacturing/transactions/pdMetalAddition/add.html"
					...
					<div class="col-xs-12">
						<div class="col-sm-2">
							<form:input path="design.styleNo" cssClass="form-control"
								onblur="javascript:popDetails();popMetalAdditionTable()" />
							<form:errors path="design.styleNo" />
						</div>
						<div class="col-sm-2">
							<form:input path="createdDate" cssClass="form-control" disabled="true" />
						...
						<table class="line-items editable table table-bordered">
							...
							<tbody>
								<tr>
									...
									<td>
										<form:input path="metalWt" id="recWtId" cssClass="form-control"
											onChange="javascript:getStock(this.value)" />
									</td>
									<td>
										<input type="text" id="vUMetalWt" name="vUMetalWt"
											class="form-control"  onChange="javascript:getUsedMetalStock(this.value)" />
									</td>
								</tr>
								<tr>
									<td colspan="10">
...
<script type="text/javascript">
	...
	$(document).ready(function(e) {
		...
		.validate({
			rules : {
				'design.styleNo' : {
					remote : {
						url : "<spring:url value='/manufacturing/transactions/styleNoAvailable.html' />",
						type : "get",
						data : {
	...
	function popDetails() {
		...
		$
				.ajax({
					url : '<spring:url value='/manufacturing/transactions/pdMetalAddition/displayBreakup.html' />?styleNo='
							+ $('#design\\.styleNo').val(),
					type : 'GET',
  ...
  $(document)
						...
						$
								.ajax({
									...
									success : function(data, textStatus, jqXHR) {
									 ...
									 if (Number(data) == -1) {
											displayMsg(event, this, 'Fresh MetalWt stock not available!');
										}else if (Number(data) == -2) {
											displayMsg(event, this , 'Used MetalWt stock not available!');
	...
	function getStock(vSel) {
		...
		if(!!vSel && !!$('#department\\.id').val() && !!$('#purity\\.id').val() && !!$('#color\\.id').val()){
	
		$
				.ajax({
					url : '<spring:url value='/manufacturing/transactions/pdMetalAddition/stock.html' />?metalWt='
							+ vSel
							+ "&locationId="
							+ $('#department\\.id').val()
							+ "&purityId="
					...
					success : function(data) {
						...
						var vMetalWt = $('#recWtId').val();
						//alert(vMetalWt);

						if (data != -1) {
							if (Number(data) < Number(vMetalWt)) {
								displayMsg(event, this, 'Fresh Metal Stock not available');
							}
	...
	function getUsedMetalStock(vSel){
		
		if(!!vSel && !!$('#department\\.id').val() && !!$('#purity\\.id').val() && !!$('#color\\.id').val()){
		
		$
		.ajax({
			url : '<spring:url value='/manufacturing/transactions/pdMetalAddition/usedMetal/stock.html' />?uMetalWt='
					+ vSel
					+ "&locationId="
					+ $('#department\\.id').val()
					+ "&purityId="
			...
			success : function(data) {
				...
				if (data != -1) {
					if (Number(data) < Number(vMetalWt)) {
						displayMsg(event, this, 'Used Metal Stock not available');
					}
	...
	function popMetalAdditionTable() {
		...
		$("#metalAdditionTableId")
				.bootstrapTable(
						'refresh',
						{
							url : "/jewels/manufacturing/transactions/pdMetalAddition/listing.html?StyleNo="
									+ $('#design\\.styleNo').val()
						});
	}
