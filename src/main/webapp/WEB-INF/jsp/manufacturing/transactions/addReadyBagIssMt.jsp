...
<div class="panel-heading" style="text-align: center;">
		...
		<div>
			<label class="col-lg-2 col-sm-2 text-left"> <span
				style="font-size: 18px;">Ready Bag Transfer</span>
			</label>
			...
			<input type="button" value="Preview" style="font-size: 14px; width: 3cm" class="btn btn-xs btn-success" id="packMtPreviewBtn" onClick="javascript:readyBagTrfListReport(2);" />
			
			
				<a class="btn btn-xs btn-info" style="font-size: 14px" type="button" id="readybagIssListBtnId"
					href="/jewels/manufacturing/transactions/readyBagIssMt.html">Listing</a> 
					
					<input type="button" value="Save" style="font-size: 14px; width: 2cm" class="btn btn-xs btn-info" id="readyBagIssMtSubmitBtn"  onclick="javascript:readyBagIssMtSave()" />
	...
	<div class="form-group">
		<form:form commandName="readyBagIssMt" id="readyBagIssMtFormId"
			action="/jewels//manufacturing/transactions/readyBagIssMt/add.html"
			cssClass="form-horizontal readyBagIssMtForm">
...
<div id="readyBagIssDtDivId" style="display: none">
	<div class="panel panel-primary" style="width: 100%">
		...
		<div class="container-fluid">
			<div class="row">
					...
					<a class="btn btn-info" style="font-size: 15px" type="button" id="readyBagPickupBtnId"	onClick="javascript:openReadyBagTrfPage()"
				...
				></span>&nbsp;Pickup</a>
					...
					<div>
						<table id="readyBagIssTblId" data-toggle="table" data-toolbar="#toolbar" data-search="true"
	...
	$('#readyBagDivId #location\\.id').chosen();
		...
		function process(date){
			...
			var dtdata='';
				$('#readyBagIssTblId').bootstrapTable({}).on('load-success.bs.table',
					function(e, data) {
				   
				dtdata = $("#readyBagIssTblId").bootstrapTable('getData').length;
		...
		function openReadyBagTrfPage(){

			window.location.href = "/jewels/manufacturing/transactions/readyBagTranfer.html?mtId="+mtid+"&mOpt=barcodeNew";
			}

		function readyBagIssMtSave(){

			if($(".readyBagIssMtForm").valid()){
				$("form#readyBagIssMtFormId").submit();
		  ...
		  $(document)
			.on(
				'submit',
				'form#readyBagIssMtFormId',
				 function(e){
					 $("#readyBagIssMtSubmitBtn").prop("disabled", true).addClass("disabled");
				});
		

		  function popReadyBagIssDtList() {
				$("#readyBagIssTblId")
						.bootstrapTable(
								'refresh',
								{
									url : "/jewels/manufacturing/transactions/readyBagIssDt/listing.html?mtId="+mtid+"&disableFlg="+disableFlg,
								});
			}

			function deleteReadyBagIssDt(e,dtId,approvalFlg){
	...
	function deleteDt(dtId){
				...
				$("#modalDialog").modal("hide");
								 ...
								 $.ajax({
										url : "/jewels/manufacturing/transactions/readyBagIssDt/delete/"+ dtId + ".html",
										type : 'GET',
	...
	function readyBagTrfListReport(val){
		...
		$
		.ajax({
			url : "/jewels/marketing/transactions/packMt/packingList/report.html?mtId="+mtid+"&tranType=readyBagTrf"+"&opt="+val,
			type : 'POST',
