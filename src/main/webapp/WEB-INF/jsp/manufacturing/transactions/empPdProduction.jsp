...
<div class="panel panel-primary" style="width: 100%">
	<div class="panel-heading" >
		<span style="font-size: 18px;"> PDC Piece Production</span>
	</div>

	<div class="form-group">
		<form:form commandName="empPdProduction"
			action="<spring:url value='/manufacturing/transactions/empPdProduction/add.html' />"
			cssClass="form-horizontal empPdProductionTransferForm">
			...
			<div class="form-group">
				...
				<label class="col-sm-2 text-right">Style No:</label>
				<div class="col-sm-2">
					...
					<table id="pcsProdDetailId" data-toggle="table"
						...
						<thead>
							<tr class="btn-primary">
								...
								<th data-field="styleNo" data-sortable="true">StyleNo</th>
...
<script type="text/javascript">
	...
	$(document).ready(function(e) {
		...
		$(".empPdProductionTransferForm").validate({
			...
			rules : {
				...
				'design.styleNo' : {
					required : true,
					remote : {
						url : "<spring:url value='/manufacturing/transactions/empPdProduction/styleNoAvailable.html' />",
						type : "get",
						data : {
	...
	function displayDetails() {
		...
		var vStyleNo = $('#design\\.styleNo').val();
			...
			//display message if required (please select the department and bagNo) ,but at present no need 5/07/2016
				...
				$("#pcsProdDetailId")
					.bootstrapTable(
							'refresh',
							{
								url : "/jewels/manufacturing/transactions/empPdProduction/listing.html?deptId="
										+ $("#department\\.id").val()
										+ "&vStyleNo="
										+ $('#design\\.styleNo').val(),
							});
	...
	function displayBreakUp() {
		...
		if (!vDeptId || !vStyleNo) {
			displayMsg(event, this,'please select the department and styleNo');
		} else {

			$
					.ajax({
						url : '<spring:url value='/manufacturing/transactions/empPdProduction/display.html' />?StyleNo='
								+ $('#design\\.styleNo').val()
								+ "&departmentId="
								+ $('#department\\.id').val(),
						type : 'GET',
						success : function(data) {
							...
							if(data === '-1'){
								displayMsg(event, this,'StyleNo not in '+ $('select[name=department\\.id] option:selected').text() +' department !');
							}else{	
								var temp = data.split("#");
	...
	function popEmployee() {
		$
				.ajax({
					url : '<spring:url value='/manufacturing/transactions/empPdProduction/employee/list.html' />?departmentId='
							+ $('#department\\.id').val(),
					type : 'GET',
					success : function(data) {
	...
	function popProdLabType() {
		...
		if (!vDeptId || !vStyleNo) {
			...
			$
					.ajax({
						url : '<spring:url value='/manufacturing/transactions/empPdProduction/prodLabType/list.html' />?departmentId='
								+ $('#department\\.id').val()
								+ "&StyleNo="
								+ $('#design\\.styleNo').val(),
						type : 'GET',
	...
	$(document)
			.on(
					...
					function(e) {
						...
						$('#vStyleNo').val($('#design\\.styleNo').val());
	...
	function editEmpPcsProduction(dtId) {
		$.ajax({
			url : "/jewels/manufacturing/transactions/empPdProduction/edit/" + dtId
					+ ".html",
			type : 'GET',
			success : function(data) {
				$("#entryForm").css('display', 'block');
	...
	function deleteEmpPcsData(empPcsPk){
		...
		$.ajax({
			...
			url:'/jewels/manufacturing/transactions/empPdProduction/delete/'+empPcsPk+'.html',
			data: 'GET',
			success : function(data){
