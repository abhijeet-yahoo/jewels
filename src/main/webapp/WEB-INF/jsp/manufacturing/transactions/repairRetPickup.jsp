...
<div class="panel panel-primary">
	...
	<label class="col-lg-5 text-left">
				<span style="font-size: 18px;">Repair/ Customer Stock</span>
			</label>
		...
		<a class="btn btn-xs btn-info" id="repairRetBackBtnId" style="font-size: 14px" type="button" onclick="javascript:repairRetBackBtn()"><b>Back</b></a>
		</div>	
	...
	<div class="panel-body">
			...
			<div class="row">
				...
				<div class="table-responsive">
 					<table id="repairRetPickupTbl" data-toggle="table" data-search="true" data-toolbar="toolbarDt" data-maintain-meta-data="true" data-height="450"  >
					<thead>
			...
			<div id="odImgDivId" class="col-md-2 center-block">
					...
					<div class="panel panel-primary" style="width:100%; height:245px">
						...
						<div class="form-group">
							<div>
								<input type="button" value="Transfer" onclick="javascript:popRepairTrfStk()" id="trfBtnId" class="btn btn-primary">
							</div>
						</div>
...
.ready(
		function(e) {
		 	...
		 	if (window.location.href.indexOf('mtid') != -1) {
						...
						repairRetMtId =vUrl.substring(window.location.href.indexOf('mtid') + 5,window.location.href.indexOf('&partyId'));

						partyId = vUrl.substring(window.location.href.indexOf('&partyId') + 9,window.location.href.indexOf('&locationId'));

						locationId = vUrl.substring(window.location.href.indexOf('&locationId') +12);
...
function popBagDt(){
				...
				.bootstrapTable(
						'refresh',
						{
							url : "/jewels/manufacturing/transactions/repairReturnTransfer/listing.html?partyId="+partyId+"&locationId="+locationId,
						}); 
...
$('#repairRetPickupTbl').on('check.bs.table', function (e, row) {
...
$('#repairRetPickupTbl').on('check-all.bs.table', function (e, row) {
...
$('#repairRetPickupTbl').on('uncheck.bs.table', function (e, row) {
...
$('#repairRetPickupTbl').on('uncheck-all.bs.table', function (e, row) {
...
$('#repairRetPickupTbl').bootstrapTable({}).on(
		'load-success.bs.table',
		function(e, data) {
			var data = JSON.stringify($("#repairRetPickupTbl").bootstrapTable('getData'));
			var vBagPcs = 0.0;
			var vWt = 0.0;
			var i=0;
...
function popSelectedQty(){
	...
	var data = $('#repairRetPickupTbl').bootstrapTable('getAllSelections');
...
function popRepairTrfStk(){
	...
	$.ajax({
		url : "/jewels/manufacturing/transactions/repairRetTransferInFactory.html?vMtId="+$('#vMtId').val()+"&repairRetMtId="+repairRetMtId,
...
function repairRetBackBtn(){
	

		window.location.href = "/jewels/manufacturing/transactions/repairRetMt/edit/"+repairRetMtId+".html"
