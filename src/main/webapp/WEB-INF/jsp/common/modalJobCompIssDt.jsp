...
<style>
	...
	<div class="modal fade" id="myJobCompIssDtModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"  >
	  <div class="modal-dialog modal-lg" role="document">
	    <div class="modal-content">
	      
	      <div class="modal-header">
		       <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> 
		      	<h4 class="modal-title" id="myModalLabel">Job Component Issue Detail</h4>
	      </div>
	      
	      <div class="modal-body">
				
				<div class="form-group" id="jobCompIssDtModalDivId">
				<form:form commandName="jobCompIssDt" id="jobCompIssDtFormId"
					action="/jewels/manufacturing/transactions/jobCompIssDt/add.html"
					cssClass="form-horizontal jobCompIssDtForm">
		...
		<div class="row">
					...
					<div class="row">
						<div class="col-lg-12">
							<form:input type="hidden" path="id" id="modalJobCompIssDtId"/>
							<input type="hidden" id="vJobCompIssMtId" name="vJobCompIssMtId" />
	      ...
	      <div class="modal-footer">
	        	<input type="button" value="Save" class="btn btn-default" onclick="javascript:popJobCompIssDtSave()" id="jobCompIssBtnId"/>
	...
	function popJobCompIssDtSave(){
		var postData = $("#jobCompIssDtFormId").serializeArray();
	
		var formURL = $("#jobCompIssDtFormId").attr("action");
		$('#jobCompIssBtnId').attr('disabled', 'disabled');
		 if($(".jobCompIssDtForm").valid()){
			...
			$.ajax({
				...
				success : function(data, textStatus, jqXHR) {
					$('#jobCompIssBtnId').removeAttr('disabled');
					if(data === '1'){
						displayInfoMsg(this,null,"Record Added Successfully !");
						popJobCompIssDt();
						
						 $('#jobCompIssDtModalDivId input[type="text"],textarea').val('');
						 $('#jobCompIssDtModalDivId input[type="number"]').val('0');
						 $('#jobCompIssDtModalDivId').find('select').val('');
						 $('#modalJobCompIssDtId').val('');
					
					}else if(data === '2'){
						 displayInfoMsg(this,null,"Record Updated Successfully !");
						 $('#myJobCompIssDtModal').modal('hide');
						 popJobCompIssDt();
	...
	function setAmountComp() {
		var metalWt = $('#jobCompIssDtModalDivId #metalWt').val();
		var rate = $('#jobCompIssDtModalDivId #rate').val();
		var defRate = rate * 1;
		var defMetalWt = metalWt *1;
		var amount = metalWt * defRate;

		$('#jobCompIssDtModalDivId #rate').val(defRate.toFixed(2));
		$('#jobCompIssDtModalDivId #amount').val(amount.toFixed(2));
		$('#jobCompIssDtModalDivId #metalWt').val(defMetalWt.toFixed(3));
