...
<style>
	...
	<div class="modal fade" id="mySaleRmCompDtModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"  >
	  <div class="modal-dialog modal-lg" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
				...
				<div class="form-group" id="saleRmCompDtModalDivId">
				<form:form commandName="saleRmCompDt" id="saleRmCompDtFormId"
					action="/jewels/marketing/transactions/saleRmCompDt/add.html"
					cssClass="form-horizontal saleRmCompDtForm">
		...
		<div class="row">
					...
					<div class="row">
						<div class="col-lg-12">
							<form:input type="hidden" path="id" id="modalSaleRmCompDtId"/>
							<input type="hidden" id="vSaleRmCompMtId" name="vSaleRmCompMtId" />
	      ...
	      <div class="modal-footer">
	        	<input type="button" value="Save" class="btn btn-default" onclick="javascript:popSaleRmCompDtSave()" id="saleRmCompBtnId"/>
	...
	function popSaleRmCompDtSave(){
		var postData = $("#saleRmCompDtFormId").serializeArray();
	
		var formURL = $("#saleRmCompDtFormId").attr("action");
		$('#saleRmCompBtnId').attr('disabled', 'disabled');
		 if($(".saleRmCompDtForm").valid()){
			...
			$.ajax({
				...
				success : function(data, textStatus, jqXHR) {
					$('#saleRmCompBtnId').removeAttr('disabled');
					if(data === '1'){
						displayInfoMsg(this,null,"Record Added Successfully !");
						popSaleRmCompDt();
						
						 $('#saleRmCompDtModalDivId input[type="text"],textarea').val('');
						 $('#saleRmCompDtModalDivId input[type="number"]').val('0');
						 $('#saleRmCompDtModalDivId').find('select').val('');
						 $('#modalSaleRmCompDtId').val('');
					
					}else if(data === '2'){
						 displayInfoMsg(this,null,"Record Updated Successfully !");
						 $('#mySaleRmCompDtModal').modal('hide');
						 popSaleRmCompDt();
	...
	function setAmountComp() {
		var metalWt = $('#saleRmCompDtModalDivId #metalWt').val();
		var rate = $('#saleRmCompDtModalDivId #rate').val();
		var defRate = rate * 1;
		var defMetalWt = metalWt *1;
		var amount = metalWt * defRate;

		$('#saleRmCompDtModalDivId #rate').val(defRate.toFixed(2));
		$('#saleRmCompDtModalDivId #amount').val(amount.toFixed(2));
		$('#saleRmCompDtModalDivId #metalWt').val(defMetalWt.toFixed(3));
