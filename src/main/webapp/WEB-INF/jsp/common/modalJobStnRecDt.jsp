...
<style>
	...
	<div class="modal fade" id="myJobStnRecDtModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"  >
	  <div class="modal-dialog modal-lg" role="document">
	    <div class="modal-content">
	      
	      <div class="modal-header">
		       <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> 
		      	<h4 class="modal-title" id="myModalLabel">Job Stone Receive Detail</h4>
	      </div>
	      
	      <div class="modal-body">
				
				<div class="form-group" id="jobStnRecDtModalDivId">
				<form:form commandName="jobStnRecDt" id="jobStnRecDtFormId"
					action="/jewels/manufacturing/transactions/jobStnRecDt/add.html"
					cssClass="form-horizontal jobStnRecDtForm">
				...
				<div class="row">
					...
					<div class="row">
						<div class="col-lg-12">
							<form:input type="hidden" path="id" id="modalJobStnRecDtId"/>
							<input type="hidden" id="vJobStnRecMtId" name="vJobStnRecMtId" />
	      ...
	      <div class="modal-footer">
	        	<input type="button" value="Save" class="btn btn-default" onclick="javascript:popJobStnRecDtSave()" id="jobStnRecBtnId"/>
	...
	function popJobStnRecDtSave(){
		
		
		var postData = $("#jobStnRecDtFormId").serializeArray();
	
		var formURL = $("#jobStnRecDtFormId").attr("action");
		$('#jobStnRecBtnId').attr('disabled', 'disabled');
		
		 if($(".jobStnRecDtForm").valid()){
				...
				success : function(data, textStatus, jqXHR) {  
					...
					if(data === '1'){
						...
						popJobStnRecDt();
						 ...
						 $('#jobStnRecDtModalDivId input[type="text"],textarea').val('');
						 $('#jobStnRecDtModalDivId input[type="number"]').val('0');
						 $('#jobStnRecDtModalDivId').find('select').val('');
						 $('#modalJobStnIssDtId').val('');
					
					}else if(data === '2'){
						 displayInfoMsg(this,null,"Record Updated Successfully !");
						 $('#myJobStnRecDtModal').modal('hide');
						 popJobStnRecDt();
						 
						 $('#jobStnRecDtModalDivId input[type="text"],textarea').val('');
						 $('#jobStnRecDtModalDivId input[type="number"]').val('0');
						 $('#jobStnRecDtModalDivId').find('select').val('');
						 $('#modalJobStnRecDtId').val('');
					
					}else{
						displayMsg(this,null,data);
					}
					
					$('#jobStnRecBtnId').removeAttr('disabled');
	...
	function setAmountStn() {
		...
		var vCarat = $('#jobStnRecDtModalDivId #carat').val();
		var vrate = Number($('#jobStnRecDtModalDivId #rate').val());
		var vDiffCarat = $('#jobStnRecDtModalDivId #diffCarat').val();
		...
		$('#jobStnRecDtModalDivId #amount').val(Number(vamount.toFixed(2)));
		$('#jobStnRecDtModalDivId #carat').val(vvCarat.toFixed(3));
		$('#jobStnRecDtModalDivId #rate').val(vvRate.toFixed(2));
		$('#jobStnRecDtModalDivId #diffCarat').val(dcarat.toFixed(3));
